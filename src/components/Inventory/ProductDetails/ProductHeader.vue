...
const hasVariantsConfigured = computed(() => {
  return Boolean(props.hasVariants) || Number(props.totalVariants || 0) > 0;
});

const variantsBadgeLabel = computed(() => {
  const total = Number(props.totalVariants || 0);
  const isCombined = Boolean(props.combineVariantAttributes);
  return `${total} variante${total === 1 ? "" : "s"}${isCombined ? " · Comb." : ""}`;
});
