<template>
  ...
  >
    <!-- Header del bloque -->
    <div class="flex justify-between gap-2 relative mb-2">
    ...
    <!-- Badge de Margen justo debajo -->
    <div
      v-if="hasCostData && productType !== 'RAW_MATERIAL'"
      class="flex flex-wrap gap-2 pb-2"
    >
      <div
        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border transition-all duration-200"
        :class="saleMarginStatus.bgClass"
        :title="saleMarginStatus.description"
      >
        <svg
          class="w-3.5 h-3.5"
          fill="none"
        ...
        <span>{{ saleMarginPercentage }}% Margen</span>
        <span class="opacity-60">•</span>
        <span>{{ saleMarginStatus.label }}</span>
      </div>
    </div>
    ...
    <div class="space-y-2">
      ...
      <!-- Markup / Margen sobre Costo (NO mostrar para RAW_MATERIAL) -->
      <!-- <div
        v-if="hasCostData && productType !== 'RAW_MATERIAL'"
      ...
      >
        ...
        <p class="text-xs text-purple-500 mt-1.5">
          Estás ganando un {{ marginPercentage }}% por encima de lo que te costó
        </p>
      </div> -->
...
// ==========================================
// COMPUTED: Rentabilidad de Venta (margen sobre precio de venta)
// ==========================================
const saleMarginPercentage = computed(() => {
  if (!hasCostData.value || !props.price) return "0.00";
  const margin = ((props.price - props.cost) / props.price) * 100;
  return margin.toFixed(2);
});
...
const saleMarginStatus = computed(() => {
  ...
  return metricsStore.getSaleMarginStatus(
    parseFloat(saleMarginPercentage.value),
  );
});
