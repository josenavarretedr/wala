  ...
  <div class="evolucion-section">
    ...
    <div class="evolucion-grid">
      <div class="evol-col">
        ...
        <div class="evol-area" v-for="area in areas" :key="`name-${area.key}`">
          <span class="evol-area-name">{{ area.title }}</span>
        </div>
      </div>

      <div v-for="period in periods" :key="`eval-${period.key}`" class="evol-col">
        <div class="evol-col-head">{{ period.label }}</div>
        <div class="evol-area" v-for="area in areas" :key="`${period.key}-${area.key}`">
          <input
    ...
    <div class="cierre-notes">
      ...
      <div class="cierre-block">
        ...
        <textarea v-model="props.closing.testimony" placeholder="Percepción del emprendedor" />
      </div>
    </div>
