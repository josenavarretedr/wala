  ...
  >
    ...
    <div class="flex items-start justify-between gap-3 mb-3">
      <div class="flex items-center gap-2 flex-wrap flex-1">
        <!-- Badge de Evento -->
        <div
          class="inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium bg-orange-50 text-orange-700 border border-orange-200"
        >
          <svg
            class="w-3 h-3"
            fill="none"
    ...
    <div class="space-y-2">
      ...
      <div class="flex items-center gap-1.5 text-xs text-gray-600 font-medium">
        ...
        <span v-if="activity.isSingleDay">
          {{ formatEventDate(activity.startDate) }}
        </span>
        <span v-else>
          {{ formatEventDate(activity.startDate) }} -
          {{ formatEventDate(activity.endDate) }}
        </span>
      </div>
...
const props = defineProps({
  ...
  participantStatus: {
    type: String,
    default: "pending", // 'attended' | 'pending'
  },
});
