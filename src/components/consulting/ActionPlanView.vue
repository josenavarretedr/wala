<template>
  <div class="space-y-8 font-display">
    ...
    <div v-if="actionsList.length > 0" class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50 border border-gray-150 rounded-[28px] p-5 shadow-inner">
      <div class="space-y-1">
        <h3 class="text-sm font-extrabold text-gray-800 uppercase tracking-wider">Avance del Plan</h3>
        <p class="text-xs text-gray-400 font-semibold leading-relaxed">
          Has completado <span class="text-emerald-600 font-black font-mono">{{ completedCount }}</span> de <span class="text-indigo-600 font-black font-mono">{{ actionsList.length }}</span> acciones recomendadas.
        </p>
      </div>

      <!-- Progress bar -->
      <div class="w-full sm:w-48 flex items-center gap-3 shrink-0">
        <div class="flex-1 bg-gray-200 h-3 rounded-full overflow-hidden border border-gray-100 shadow-inner">
          <div
            class="bg-gradient-to-r from-emerald-500 to-indigo-600 h-full rounded-full transition-all duration-550 ease-out"
    ...
    <div v-if="actionsList.length > 0" class="space-y-8">
      <div
        v-for="group in groupedActions"
        :key="group.areaKey"
        class="bg-white border border-gray-150/70 rounded-[32px] p-5 sm:p-6 shadow-sm hover:shadow-md transition-all duration-300 space-y-5"
      >
        <!-- Area Title Header -->
        <div class="flex items-center justify-between border-b border-gray-100 pb-3">
          <div class="flex items-center gap-3.5">
            <div class="p-3 bg-[#E35336]/10 text-[#E35336] rounded-2xl border border-[#E35336]/20">
              <component :is="getAreaIcon(group.icon)" class="w-6 h-6 stroke-[2.2]" />
            </div>
            <div>
          ...
          <span class="text-[10px] font-black font-mono bg-indigo-50 border border-indigo-150 text-indigo-700 px-2 py-0.5 rounded-lg shadow-sm">
          ...
          <div
            ...
            :class="[
              action.status === 'completed'
                ...
                : 'border-gray-200 hover:border-indigo-150 hover:bg-indigo-50/5'
            ]"
          >
            ...
            <button
              ...
              :class="[
                action.status === 'completed'
                  ...
                  : 'border-gray-300 bg-white hover:border-indigo-500 hover:shadow-sm'
              ]"
            >
              <Check v-if="action.status === 'completed'" class="w-3.5 h-3.5 stroke-[3.5] animate-scaleIn" />
            </button>
            ...
            <div class="flex-1 min-w-0 space-y-2">
              ...
              <!-- Meta Footer -->
              <div class="flex items-center gap-2.5 flex-wrap text-[9px] font-black">
                <span class="text-indigo-650 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-md">
                  {{ action.frequency }}
                </span>
                <span class="text-gray-500 bg-gray-50 border border-gray-250/50 px-2 py-0.5 rounded-md">
                  {{ action.walaSection }}
                </span>
    ...
    >
      <div class="mx-auto w-12 h-12 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center shadow-inner">
        <component :is="getAreaIcon('Book')" class="w-6 h-6 stroke-[2.2]" />
      </div>
      <div class="space-y-1.5">
        <h4 class="text-sm font-black text-gray-800 uppercase tracking-wider">Sin Plan de Acción Activo</h4>
        <p class="text-xs text-gray-450 font-semibold leading-relaxed max-w-sm mx-auto">
          Aún no se ha estructurado un Plan de Acción para este negocio. Los planes de acción guían al emprendedor a realizar tareas concretas en sus áreas críticas de mejora.
        </p>
      </div>
    ...
    <div v-else class="py-12 flex flex-col items-center justify-center gap-3">
      ...
      <span class="text-xs font-bold text-gray-400">Actualizando plan de acción...</span>
    </div>
  </div>
</template>
