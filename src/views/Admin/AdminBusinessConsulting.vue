...
import { useAdminBusinessConsultingStore } from "@/stores/adminBusinessConsultingStore";
...
function getAdminDossierStoreWithSaveAction() {
  let store = useAdminBusinessConsultingStore();
  ...
  store.$dispose();
  store = useAdminBusinessConsultingStore();
...
const businessId = computed(() => route.query.businessId);
...
function goBack() { router.push(/admin/users); }/consultings`);
}
...
async function refreshAfterCycleAction() {
  const dossierStore = getAdminDossierStoreWithSaveAction();
...
async function hydrateHeaderData() {
  try {
    const dossierStore = useAdminBusinessConsultingStore();
    const dossier = await dossierStore.loadDossier(businessId.value);
    hydrateDraft(dossier);
    resetBaseline();
  } catch (error) {
...
async function saveDraft() {
  ...
  try {
    ...
    await dossierStore.saveProgramConsultingDossier(businessId.value, );
