  ...
  <div class="max-w-3xl mx-auto p-4 sm:p-6 pb-24">
    ...
    <div class="text-center mb-8">
      ...
      <p class="text-xs text-gray-400">
        Configura el costo de materias primas e insumos por artículo
      </p>
    </div>
        ...
        <div class="flex gap-3">
          ...
          <div class="text-sm text-emerald-800">
            ...
            <p class="text-emerald-700">
              ...
              un flujo completo para calcular y gestionar los costos de
              materiales.
            </p>
          </div>
        </div>
      ...
      <div class="mb-6">
        <label class="block text-sm font-medium text-gray-700 mb-2">
          <div class="flex items-center gap-2">
            ...
            <span>Costo de Materiales (S/)</span>
          </div>
        </label>
        <div class="relative">
          <span
          ...
          <input
            ...
            min="0"
            placeholder="0.00"
            class="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-lg font-medium"
            @input="hasChanges = true"
          />
        </div>
        <p class="text-xs text-gray-500 mt-2">
          Ingresa el costo total de materiales por unidad de producto
        </p>
      </div>

      <!-- Vista previa del valor actual -->
      <div
        v-if="costingStore.hasMaterialsCost && !hasChanges"
    ...
    <div
      v-if="showSuccess"
      class="fixed bottom-6 right-6 bg-emerald-500 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 animate-slide-up"
    >
      <svg
        class="w-6 h-6"
        fill="none"
...
const loadCurrentCost = () => {
  const currentCost = costingStore.getCostByType("materials");
...
const handleSave = async () => {
  ...
  try {
    // Actualizar el store
    costingStore.updateMaterialsCost(localCost.value);
