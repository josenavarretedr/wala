...
export default function ValidationPanel() {
  ...
  return (
    <div className={`p-5 rounded-2xl border backdrop-blur-md transition-all duration-300 ${
      isValid 
        ? 'bg-emerald-50/50 border-emerald-250 dark:bg-emerald-950/15 dark:border-emerald-500/20' 
        : 'bg-rose-50/50 border-rose-250 dark:bg-rose-950/15 dark:border-rose-500/20'
    }`}>
      <div className="flex items-center gap-2.5 mb-3 border-b border-slate-200 dark:border-slate-800/40 pb-2.5">
        {isValid ? (
      ...
      <div className="space-y-3">
        ...
        {errors.length > 0 && (
          <div className="space-y-1.5">
            ...
            <div className="space-y-1">
              {errors.map((err, i) => (
                <div key={i} className="text-xs text-rose-800 flex items-start gap-1.5 bg-rose-100/50 p-2 rounded border border-rose-200 dark:bg-rose-500/5 dark:text-rose-200/90 dark:border-rose-500/10">
        ...
        {warnings.length > 0 && (
          <div className="space-y-1.5">
            ...
            <div className="space-y-1">
              {warnings.map((warn, i) => (
                <div key={i} className="text-xs text-amber-800 flex items-start gap-1.5 bg-amber-100/50 p-2 rounded border border-amber-200 dark:bg-amber-500/5 dark:text-amber-200/90 dark:border-amber-500/10">
        ...
        {isValid && errors.length === 0 && (
          <div className="flex items-center gap-2 p-3 bg-emerald-100/60 rounded-xl border border-emerald-200 text-emerald-800 dark:bg-emerald-500/5 dark:border-emerald-500/10 dark:text-emerald-400">
